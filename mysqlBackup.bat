REM Original by Bradford Mitchell 
REM  - posted July 18, 2008 @ 7:11 pm
REM  - dev.mysql.com/doc/refman/5.5/en/mysqldump.html (in the comments)
REM
...
IF NOT EXIST %bkupdir%\INSTALLED.OK (
  @ECHO DIRECTORY STRUCTURE NOT IN PLANCE. >> %bkupdir%\dbBkup_%dbnamesf%_%yy%%mm%%dd%.log
  @ECHO PLEANS RUN %0 --INSTALL OR %0 --CREATEDIRS >> %bkupdir%\dbBkup_%dbnamesf%_%yy%%mm%%dd%.log
...
REM This EXIST check to ensure we only create 1 weekly backup
